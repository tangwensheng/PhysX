...
EXPERIMENT=complex_trimesh_presort_hold_r4_${STAMP}
...

{
	...
	date
	grep -n "PX_DCU_NP_TRIMESH_PRESORT_HOLD_MS\|DCU NP TRIMESH PRESORT HOLD" "${CPP}"
	host_probe_exit=$?
...
if [ ${build_gate} -eq 1 ] && [ ${preflight_gate} -eq 1 ]; then
	...
	if [ ${run_cd_exit} -eq 0 ]; then
		env \
			...
			PX_DCU_NP_TRIMESH_PRESORT_HOLD_MS=90000 \
	...
	(
		...
		while kill -0 "${app_pid}" 2>/dev/null; do
			sample=$((sample + 1))
			if grep -q '\[DCU NP TRIMESH PRESORT HOLD\] phase=pre-exit' "${RUN_LOG}"; then
				phase=pre-exit
			elif grep -q '\[DCU NP TRIMESH PRESORT HOLD\] phase=begin' "${RUN_LOG}"; then
				phase=hold
			else
				phase=startup
			fi
	...
	echo "run_exit=${run_exit} expected_run_exit=93 monitor_wait_exit=${monitor_wait_exit} watchdog_wait_exit=${watchdog_wait_exit} watchdog_fired=${watchdog_fired}" >> "${RUN_LOG}"
...
grep -q '\[DCU NP TRIMESH PRESORT HOLD\] phase=ready ' "${RUN_LOG}"
hold_ready_exit=$?
grep -q '\[DCU NP TRIMESH PRESORT HOLD\] phase=begin ms=90000' "${RUN_LOG}"
hold_begin_exit=$?
grep -q '\[DCU NP TRIMESH PRESORT HOLD\] phase=pre-exit ms=10000' "${RUN_LOG}"
hold_pre_exit_marker_exit=$?
grep -q '\[DCU NP TRIMESH PRESORT HOLD\] phase=end totalMs=100000' "${RUN_LOG}"
hold_end_exit=$?
if grep -q '\[DCU NP TRIMESH TINYSORT PACKED\]' "${RUN_LOG}"; then
	post_sort_marker_absent=0
else
	post_sort_marker_absent=1
fi
...
if [ ${run_exit} -eq 93 ] && [ ${hold_ready_exit} -eq 0 ] && [ ${hold_begin_exit} -eq 0 ] && \
	[ ${hold_pre_exit_marker_exit} -eq 0 ] && [ ${hold_end_exit} -eq 0 ] && [ ${post_sort_marker_absent} -eq 1 ] && \
	...
	probe_exit=0
	hypothesis_result=presort-path-stable-sort-leading-trigger
elif [ ${run_exit} -eq 137 ] && [ ${alive_health_failures} -gt 0 -o ${post_health_failures} -gt 0 ]; then
	probe_exit=1
	hypothesis_result=presort-or-earlier-device-failure
...
echo "hold_ready_exit=${hold_ready_exit} hold_begin_exit=${hold_begin_exit} hold_pre_exit_marker_exit=${hold_pre_exit_marker_exit} hold_end_exit=${hold_end_exit} post_sort_marker_absent=${post_sort_marker_absent}" >> "${RUN_LOG}"
