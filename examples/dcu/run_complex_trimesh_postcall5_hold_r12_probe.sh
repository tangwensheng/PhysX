...
EXPERIMENT=complex_trimesh_postcall5_hold_r12_${STAMP}
...

{
	...
	date
	grep -n "PX_DCU_NP_TRIMESH_POSTFUNCTION_PROBE\|PX_DCU_NP_TRIMESH_POSTFUNCTION_HOLD_MS\|DCU NP TRIMESH POSTFUNCTION HOLD" "${CPP}"
	host_probe_exit=$?
...
if [ ${build_gate} -eq 1 ] && [ ${preflight_gate} -eq 1 ]; then
	...
	if [ ${run_cd_exit} -eq 0 ]; then
		env \
			...
			PX_DCU_NP_TRIMESH_POSTFUNCTION_PROBE=5 \
			PX_DCU_NP_TRIMESH_POSTFUNCTION_HOLD_MS=30000 \
	...
	(
		...
		while kill -0 "${app_pid}" 2>/dev/null; do
			sample=$((sample + 1))
			if grep -q '\[DCU NP TRIMESH POSTFUNCTION HOLD\] phase=pre-exit' "${RUN_LOG}"; then phase=pre-exit
			elif grep -q '\[DCU NP TRIMESH POSTFUNCTION HOLD\] phase=begin' "${RUN_LOG}"; then phase=hold
	...
	echo "run_exit=${run_exit} expected_run_exit=99 sync_failure_exit=98 monitor_wait_exit=${monitor_wait_exit} watchdog_wait_exit=${watchdog_wait_exit} watchdog_fired=${watchdog_fired}" >> "${RUN_LOG}"
	post_sample=0
...
grep -Eq '\[DCU NP TRIMESH POSTFUNCTION HOLD\] phase=ready launch=5 sync=0 tests=[0-9]+' "${RUN_LOG}"
hold_ready_exit=$?
grep -q '\[DCU NP TRIMESH POSTFUNCTION HOLD\] phase=begin ms=30000' "${RUN_LOG}"
hold_begin_exit=$?
grep -Eq '\[DCU NP TRIMESH POSTFUNCTION HOLD\] phase=pre-exit ms=10000 actualHoldMs=[0-9]+' "${RUN_LOG}"
hold_pre_exit_marker_exit=$?
grep -Eq '\[DCU NP TRIMESH POSTFUNCTION HOLD\] phase=end requestedTotalMs=40000 actualTotalMs=[0-9]+' "${RUN_LOG}"
hold_end_exit=$?
actual_hold_ms=$(sed -n 's/.*phase=pre-exit ms=10000 actualHoldMs=\([0-9][0-9]*\).*/\1/p' "${RUN_LOG}" | tail -n 1)
actual_total_ms=$(sed -n 's/.*phase=end requestedTotalMs=40000 actualTotalMs=\([0-9][0-9]*\).*/\1/p' "${RUN_LOG}" | tail -n 1)
if [ -n "${actual_hold_ms}" ] && [ ${actual_hold_ms} -ge 30000 ] && [ ${actual_hold_ms} -le 35000 ]; then hold_timing_valid=1; else hold_timing_valid=0; fi
if [ -n "${actual_total_ms}" ] && [ ${actual_total_ms} -ge 40000 ] && [ ${actual_total_ms} -le 45000 ]; then total_timing_valid=1; else total_timing_valid=0; fi
...
if [ ${run_exit} -eq 99 ] && [ ${hold_ready_exit} -eq 0 ] && [ ${hold_begin_exit} -eq 0 ] && \
	...
	probe_exit=0
	hypothesis_result=call-5-function-insufficient-trigger-is-after-function-return-or-other-stream
elif [ ${run_exit} -eq 98 ]; then
	probe_exit=1
	hypothesis_result=call-5-function-stream-synchronize-detected-failure
elif [ ${alive_health_failures} -gt 0 -o ${post_health_failures} -gt 0 ]; then
	probe_exit=1
	hypothesis_result=call-5-function-completion-sufficient-for-device-failure
