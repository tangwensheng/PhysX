...
EXPERIMENT=complex_trimesh_precall6_hold_r11_${STAMP}
...
if [ ${build_gate} -eq 1 ] && [ ${preflight_gate} -eq 1 ]; then
	...
	if [ ${run_cd_exit} -eq 0 ]; then
		env \
			...
			PX_DCU_NP_TRIMESH_PREMIDPHASE_PROBE=6 \
...
grep -Eq '\[DCU NP TRIMESH PREMIDPHASE HOLD\] phase=ready launch=6 .*zero=1' "${RUN_LOG}"
hold_ready_exit=$?
...
if [ ${run_exit} -eq 96 ] && [ ${hold_ready_exit} -eq 0 ] && [ ${hold_begin_exit} -eq 0 ] && \
	...
	probe_exit=0
	hypothesis_result=first-five-trimesh-calls-stable-call-6-introduces-device-failure
elif [ ${alive_health_failures} -gt 0 -o ${post_health_failures} -gt 0 ]; then
	probe_exit=1
	hypothesis_result=trimesh-call-5-introduces-device-failure
