...
EXPERIMENT=complex_trimesh_precore_hold_r5_${STAMP}
...

{
	...
	date
	grep -n "PX_DCU_NP_TRIMESH_PRECORE_PROBE\|PX_DCU_NP_TRIMESH_PRECORE_HOLD_MS\|DCU NP TRIMESH PRECORE HOLD" "${CPP}"
	host_probe_exit=$?
...
if [ ${build_gate} -eq 1 ] && [ ${preflight_gate} -eq 1 ]; then
	...
	if [ ${run_cd_exit} -eq 0 ]; then
		env \
			...
			PX_DCU_NP_TRIMESH_PRECORE_PROBE=9 \
			PX_DCU_NP_TRIMESH_PRECORE_HOLD_MS=90000 \
	...
	(
		...
		while kill -0 "${app_pid}" 2>/dev/null; do
			sample=$((sample + 1))
			if grep -q '\[DCU NP TRIMESH PRECORE HOLD\] phase=pre-exit' "${RUN_LOG}"; then
				phase=pre-exit
			elif grep -q '\[DCU NP TRIMESH PRECORE HOLD\] phase=begin' "${RUN_LOG}"; then
				phase=hold
			else
				phase=startup
			fi
	...
	watchdog_wait_exit=$?
	if grep -q '^watchdog_fired=1 ' "${RUN_LOG}"; then
		watchdog_fired=1
	fi
	echo "run_exit=${run_exit} expected_run_exit=94 monitor_wait_exit=${monitor_wait_exit} watchdog_wait_exit=${watchdog_wait_exit} watchdog_fired=${watchdog_fired}" >> "${RUN_LOG}"
...
grep -Eq '\[DCU NP TRIMESH PRECORE HOLD\] phase=ready .*valid=1' "${RUN_LOG}"
hold_ready_exit=$?
grep -q '\[DCU NP TRIMESH PRECORE HOLD\] phase=begin ms=90000' "${RUN_LOG}"
hold_begin_exit=$?
grep -q '\[DCU NP TRIMESH PRECORE HOLD\] phase=pre-exit ms=10000' "${RUN_LOG}"
hold_pre_exit_marker_exit=$?
grep -q '\[DCU NP TRIMESH PRECORE HOLD\] phase=end totalMs=100000' "${RUN_LOG}"
hold_end_exit=$?
if grep -q '\[DCU NP TRIMESH ORDERED PREFIX\]\|\[DCU NP TRIMESH TINYSORT PACKED\]' "${RUN_LOG}"; then
	post_core_marker_absent=0
else
	post_core_marker_absent=1
fi
...
if [ ${run_exit} -eq 94 ] && [ ${hold_ready_exit} -eq 0 ] && [ ${hold_begin_exit} -eq 0 ] && \
	[ ${hold_pre_exit_marker_exit} -eq 0 ] && [ ${hold_end_exit} -eq 0 ] && [ ${post_core_marker_absent} -eq 1 ] && \
	[ ${alive_health_failures} -eq 0 ] && [ ${hold_healthy_samples} -gt 0 ] && \
	[ ${pre_exit_healthy_samples} -gt 0 ] && [ ${post_health_failures} -eq 0 ] && [ ${watchdog_fired} -eq 0 ]; then
	probe_exit=0
	hypothesis_result=precore-stable-core-leading-trigger
elif [ ${run_exit} -eq 137 ] && [ ${alive_health_failures} -gt 0 -o ${post_health_failures} -gt 0 ]; then
	probe_exit=1
	hypothesis_result=midphase-or-earlier-device-failure
	...
	probe_exit=1
	hypothesis_result=invalid-midphase-layout
...
echo "hold_ready_exit=${hold_ready_exit} hold_begin_exit=${hold_begin_exit} hold_pre_exit_marker_exit=${hold_pre_exit_marker_exit} hold_end_exit=${hold_end_exit} post_core_marker_absent=${post_core_marker_absent}" >> "${RUN_LOG}"
