...
EXPERIMENT=complex_trimesh_precore5_hold_r16_${STAMP}
...
grep -Eq '\[DCU NP TRIMESH PRECORE HOLD\] phase=ready launch=5 sync=0 copy=0 tests=[0-9]+ .*valid=1' "${RUN_LOG}"
hold_ready_exit=$?
...
if [ ${run_skipped} -eq 1 ]; then
	...
	probe_exit=0
	hypothesis_result=through-midphase-insufficient-trigger-is-core
elif [ ${run_exit} -eq 95 ]; then
	probe_exit=1
	hypothesis_result=midphase-sync-copy-or-layout-validation-failed
elif [ ${alive_health_failures} -gt 0 -o ${post_health_failures} -gt 0 ]; then
	probe_exit=1
	hypothesis_result=through-midphase-sufficient-for-device-failure
