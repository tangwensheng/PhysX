...
EXPERIMENT=complex_trimesh_postsort5_hold_r13_${STAMP}
...

{
	...
	date
	grep -n "PX_DCU_NP_TRIMESH_POSTSORT_PROBE\|PX_DCU_NP_TRIMESH_POSTSORT_HOLD_MS\|DCU NP TRIMESH POSTSORT HOLD" "${CPP}"
	host_probe_exit=$?
...
if [ ${build_gate} -eq 1 ] && [ ${preflight_gate} -eq 1 ]; then
	...
	if [ ${run_cd_exit} -eq 0 ]; then
		env \
			...
			PX_DCU_NP_TRIMESH_POSTSORT_PROBE=5 \
			PX_DCU_NP_TRIMESH_POSTSORT_HOLD_MS=30000 \
	...
	(
		...
		while kill -0 "${app_pid}" 2>/dev/null; do
			sample=$((sample + 1))
			if grep -q '\[DCU NP TRIMESH POSTSORT HOLD\] phase=pre-exit' "${RUN_LOG}"; then phase=pre-exit
			elif grep -q '\[DCU NP TRIMESH POSTSORT HOLD\] phase=begin' "${RUN_LOG}"; then phase=hold
	...
	echo "run_exit=${run_exit} expected_run_exit=100 sync_failure_exit=101 monitor_wait_exit=${monitor_wait_exit} watchdog_wait_exit=${watchdog_wait_exit} watchdog_fired=${watchdog_fired}" >> "${RUN_LOG}"
	post_sample=0
...
grep -Eq '\[DCU NP TRIMESH POSTSORT HOLD\] phase=ready launch=5 sync=0 tests=[0-9]+' "${RUN_LOG}"
hold_ready_exit=$?
grep -q '\[DCU NP TRIMESH POSTSORT HOLD\] phase=begin ms=30000' "${RUN_LOG}"
hold_begin_exit=$?
grep -Eq '\[DCU NP TRIMESH POSTSORT HOLD\] phase=pre-exit ms=10000 actualHoldMs=[0-9]+' "${RUN_LOG}"
hold_pre_exit_marker_exit=$?
grep -Eq '\[DCU NP TRIMESH POSTSORT HOLD\] phase=end requestedTotalMs=40000 actualTotalMs=[0-9]+' "${RUN_LOG}"
hold_end_exit=$?
...
if [ ${run_exit} -eq 100 ] && [ ${hold_ready_exit} -eq 0 ] && [ ${hold_begin_exit} -eq 0 ] && \
	...
	probe_exit=0
	hypothesis_result=through-sort-insufficient-trigger-is-postprocess-or-later
elif [ ${run_exit} -eq 101 ]; then
	probe_exit=1
	hypothesis_result=through-sort-stream-synchronize-detected-failure
elif [ ${alive_health_failures} -gt 0 -o ${post_health_failures} -gt 0 ]; then
	probe_exit=1
	hypothesis_result=through-sort-sufficient-for-device-failure
